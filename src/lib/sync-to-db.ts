...
async function syncEmailsToDatabase(emails: EmailMessage[], accountId: string) {
    ...
    // If no emails to sync, just return early
    if (emails.length === 0) {
        console.log('No emails to sync, skipping database operations');
        return;
    }
    ...
    try {
        ...
        async function syncToOrama() {
            console.log('Starting Orama sync...');
            ...
            console.log('Orama sync completed');
        }

        async function syncToDB() {
            console.log('Starting database sync...');
            for (const [index, email] of emails.entries()) {
                console.log(`Processing email ${index + 1}/${emails.length}: ${email.subject}`);
...
async function upsertEmail(email: EmailMessage, index: number, accountId: string) {
    ...
    try {
        ...
        console.log(`Email ${index + 1}: Label type = ${emailLabelType}`);
        ...
        console.log(`Email ${index + 1}: Upserting ${addressesToUpsert.size} email addresses`);
        ...
        console.log(`Email ${index + 1}: Addresses processed - From: 1, To: ${toAddresses.length}, CC: ${ccAddresses.length}, BCC: ${bccAddresses.length}, ReplyTo: ${replyToAddresses.length}`);

        // 2. Upsert Thread
        console.log(`Email ${index + 1}: Upserting thread ${email.threadId}`);
        ...
        console.log(`Email ${index + 1}: Thread upserted successfully`);

        // 3. Upsert Email
        console.log(`Email ${index + 1}: Upserting email ${email.id}`);
        ...
        console.log(`Email ${index + 1}: Email upserted successfully`);
        ...
        console.log(`Email ${index + 1}: Thread now has ${threadEmails.length} emails`);
        ...
        console.log(`Email ${index + 1}: Completed successfully`);
    } catch (error) {
        console.error(`Error upserting email ${index + 1}:`, error);
        throw error;
    }
}
